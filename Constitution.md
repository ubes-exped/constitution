...
8. However, it is recognized that the Single Transferable Vote ("STV") system of voting is typically a fairer way of electing candidates, though requires a more intensive counting process. If one or more of the following apply prior to the commencement of each election, then STV must be used for that election and it shall not be permitted to elect that position on the basis of an election conducted using the FPTP system of voting. These criteria shall apply on a per-position basis, so different systems of voting may be used to elect different positions within one General Meeting. It shall not be permitted to change retrospectively the system of voting after the commencement of that election. There shall be no limit as to the number of elections conducted using either system of voting, provided the criteria for doing so are duly observed.
   ...
   2. Any eligible voter interrupts proceedings to request the use of the STV system of voting for that election, provided that voter can demonstrate support from a number of eligible voters (including  themselves) of no less than 10% of the quorum threshold (as defined by 8(d)). For the avoidance of doubt, this refers to 10% of threshold required for quoracy, not 10% of the number of eligible voters present.
...
10. The system of voting used for each election shall be recorded in the minutes of the General Meeting. Where STV is used as the system of voting, it shall also be recorded which criterion (or criteria) applied prior to the commencement of that election.
