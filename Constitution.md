...
8. However, it is recognized that the Single Transferable Vote ("STV") system of voting is typically a fairer way of electing candidates, though requires a more intensive counting process. If one or more of the following apply prior to the commencement of each vote, then STV must be used for that vote and it shall not be permitted to elect that position on the basis of a vote conducted using the FPTP system. These criteria shall apply on a per-position basis, so different voting systems may be used to elect different positions within one General Meeting. It shall not be permitted to change retrospectively the system of voting after the commencement of that vote. There shall be no limit as to the number of votes conducted using either voting system, provided the criteria for doing so are duly observed.
   1. The Chairperson decides in their sole authority to require the use of the STV voting system for that vote, and/or
   2. Any eligible voter interrupts proceedings to object and request the use of the STV voting system for that vote, provided that voter can demonstrate support from a number of eligible voters (including  themselves) of no less than 10% of the quorum threshold (as defined by 8(d)). For the avoidance of doubt, this refers to 10% of threshold required for quoracy, not 10% of the number of voters present.
...
10. The voting system used for each vote shall be recorded in the minutes of the General Meeting. Where STV is used as the voting system, it shall also be recorded which criterion (or criteria) applied prior to the commencement of that vote.
...
12. If re-open nominations is elected in a multi-seat election where STV is used as the voting system, its surplus of votes and any other votes for re-open nominations will be transferred to a new re-open nominations option.
