...
7. The default system of voting shall be First Past the Post ("FPTP"), for ease of counting only. Where FPTP is used as the system of voting, it shall be defined as follows. Eligible voters shall cast (up to) the same number of votes as there are positions to be filled. Each vote shall be a mark next the selected candidate(s). In a multi-position election, each voter may not vote more than once for any candidate. The number of votes received for each candidate is counted, and the highest-polling candidate(s) corresponding to the number of positions to be filled are elected.
8. However, it is recognized that the Single Transferable Vote ("STV") system of voting is typically a fairer way of electing candidates, though requires a more intensive counting process. If one or more of the following apply prior to the commencement of each election, then STV must be used as the system of voting for that election instead of FPTP. These criteria shall apply on a per-role basis, so different systems of voting may be used for each election within one General Meeting. There shall be no limit as to the number of elections conducted using either system of voting, provided the criteria for doing so are duly observed.
...
9. The Chairperson must give eligible voters a reasonable opportunity to request the use of STV prior to the commencement of each election. It shall not be permitted to change retrospectively the system of voting after the commencement of that election.
10. In the event that the STV system of voting is used, it shall be used as defined by the Electoral Reform Society of Great Britain and Northern Ireland ('How to conduct an election by the Single Transferable Vote', 3rd Edition, June 1997).
11. The system of voting used for each election shall be recorded in the minutes of the General Meeting. Where STV is used as the system of voting, it shall also be recorded which criterion (or criteria) applied prior to the commencement of that election.
12. All ballots must include the option to re-open nominations. Should re-open nominations be elected, the post will remain vacant until a by-election is held.
13. If re-open nominations is elected in a multi-seat election where STV is used as the system of voting, its surplus of votes and any other votes for re-open nominations will be transferred to a new re-open nominations option.
14. For the avoidance of doubt, the only roles which are to be elected as a multi-position election are those defined in Section 5 (Committee) which include "(_n_ positions)" in their definition, where _n_ is the number of positions to be elected. All other roles (including but not limited to President and Vice-President) are to be elected as a series of single-position elections.
15. The Chairperson of the General Meeting to have the casting vote in the event of a tie, having not voted previously.
