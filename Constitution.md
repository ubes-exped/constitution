...
7. The default system of voting shall be First Past the Post ("FPTP"), for ease of counting only. Eligible voters shall cast (up to) the same number of votes as there are positions to be filled. Each vote shall be a mark next the selected candidate(s). In a multi-position election, each voter may not vote more than once for any candidate. The number of votes received for each candidate is counted, and the highest-polling candidate(s) corresponding to the number of positions to be filled are elected.
8. However, it is recognized that the Single Transferable Vote ("STV") system of voting is typically a fairer way of electing candidates, though requires a more intensive counting process. If one or more of the following apply prior to the commencement of each vote, then STV must be used for that vote and it shall not be permitted to elect that position on the basis of a vote conducted using the FPTP system of voting. These criteria shall apply on a per-position basis, so different systems of voting may be used to elect different positions within one General Meeting. It shall not be permitted to change retrospectively the system of voting after the commencement of that vote. There shall be no limit as to the number of votes conducted using either system of voting, provided the criteria for doing so are duly observed.
   1. The Chairperson decides in their sole authority to require the use of the STV system of voting for that vote, and/or
   2. Any eligible voter interrupts proceedings to request the use of the STV system of voting for that vote, provided that voter can demonstrate support from a number of eligible voters (including  themselves) of no less than 10% of the quorum threshold (as defined by 8(d)). For the avoidance of doubt, this refers to 10% of threshold required for quoracy, not 10% of the number of eligible voters present.
...
10. The system of voting used for each vote shall be recorded in the minutes of the General Meeting. Where STV is used as the system of voting, it shall also be recorded which criterion (or criteria) applied prior to the commencement of that vote.
11. All ballots must include the option to re-open nominations. Should re-open nominations be elected, the post will remain vacant until a by-election is held.
12. If re-open nominations is elected in a multi-seat election where STV is used as the system of voting, its surplus of votes and any other votes for re-open nominations will be transferred to a new re-open nominations option.
13. For the avoidance of doubt, the only positions which are to be elected as a multi-position election are those defined in Section 5 (Committee) which include "(_n_ positions)" in their definition, where _n_ is the number of positions to be elected. All other roles (including but not limited to President and Vice-President) are to be elected as a series of single-position elections.
